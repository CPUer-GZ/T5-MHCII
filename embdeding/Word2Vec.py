...
def get_embeddings(dataset):
    ...
    for idx in tqdm(range(total_sequences), desc="Processing sequences"):
        ...
        with torch.no_grad():
            ...
            peptide_embeds = np.mean(peptide_embeds, axis=0) 

        with torch.no_grad():
            mhcii_embeds = embedder.embed(mhcii_seq)
            mhcii_embeds = np.mean(mhcii_embeds, axis=0)  

        peptide_embs.append(peptide_embeds)
        mhcii_embs.append(mhcii_embeds)
...
output_h5_path = "/home/data/representation_data/Word2Vec.h5"
